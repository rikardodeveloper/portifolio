...
export class CustomerController {
    ...
    public async addCustomer(@Body({ validate: true }) customerParam: CreateCustomer, @Res() response: any): Promise<any> {
        ...
        if (resultUser) {
            const successResponse: any = {
                status: 1,
                message: 'Este email já esta registrado em nosso banco de dados.',
            };
        ...
        if (customerParam.password === customerParam.confirmPassword) {
            ...
            newCustomer.phone = customerParam.phone;
            newCustomer.address = customerParam.address;
            newCustomer.address_numero = customerParam.address_numero;
            newCustomer.local = customerParam.local;
            newCustomer.tipo_cadastro = customerParam.tipo_cadastro;
            newCustomer.cnpj_cpf = customerParam.cnpj_cpf;
            newCustomer.inscr_estad_rg = customerParam.inscr_estad_rg;
            newCustomer.address_complem = customerParam.address_complem;
            newCustomer.contato_resp = customerParam.contato_resp;
            newCustomer.email_pre_impres = customerParam.email_pre_impres;
            newCustomer.email_financeiro = customerParam.email_financeiro;
            newCustomer.website = customerParam.website;
            newCustomer.bairro = customerParam.bairro;
    ...
    public async updateCustomer(@Param('id') id: number, @Body({ validate: true }) customerParam: UpdateCustomer, @Res() response: any): Promise<any> {
        ...
        if (customerParam.password === customerParam.confirmPassword) {
            ...
            customer.address_numero = customerParam.address_numero;
            customer.local = customerParam.local;
            customer.tipo_cadastro = customerParam.tipo_cadastro;
            customer.cnpj_cpf = customerParam.cnpj_cpf;
            customer.inscr_estad_rg = customerParam.inscr_estad_rg;
            customer.address_complem = customerParam.address_complem;
            customer.contato_resp = customerParam.contato_resp;
            customer.email_pre_impres = customerParam.email_pre_impres;
            customer.email_financeiro = customerParam.email_financeiro;
            customer.website = customerParam.website;
            customer.bairro = customerParam.bairro;
            ////////////
            ...
            if (customerSave) {
                ...
                if (customerParam.status === 1) {
                    const emailContent = await this.emailTemplateService.findOne(4);
                    const message = emailContent.content.replace('{name}', customerParam.username).replace('{username}', customerParam.email);
                    MAILService.customerLoginMail(message, customerParam.email, emailContent.subject);
                }
